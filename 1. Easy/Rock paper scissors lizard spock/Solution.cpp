...
struct Player
{
    Player(int p_num = 0, char p_sign = ' ') :
           num(p_num), sign(p_sign), rounds(vector<int>()) {}
           
    bool operator<(Player& p2)
    {
        rounds.emplace_back(p2.num);
        p2.rounds.emplace_back(num);
        
        if (find(wins[sign].begin(), wins[sign].end(), p2.sign)
                != wins[sign].end()) return true;
        else if (find(wins[p2.sign].begin(), wins[p2.sign].end(), sign)
                != wins[p2.sign].end()) return false;
        else return (num < p2.num);
    }
...
int main()
{
    ...
    for (int i = 0; i < N; ++i) 
    {
        ...
        contestants.emplace_back(Player(num, sign));
    }
    ...
    while (contestants.size() > 1)
    {
        ...
        for (int i = 0; i < contestants.size() - 1; i += 2)
        {
            ...
            if (p1 < p2) current.emplace_back(p1);
            else         current.emplace_back(p2);
        }
